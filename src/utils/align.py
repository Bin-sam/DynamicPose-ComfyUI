...
def align_process(ref_image, ref_image_pts, pose_images, pose_video_pts, dst_size=(512, 768)):
    # ref image align to dst_size
    ref_img = ref_image[0].numpy() * 255
    h, w, _ = ref_img.shape
    src_pts = np.array([[w/2.0, 0], [w/2.0, h-1]])
    dst_w, dst_h = dst_size
    dst_pts = np.array([[dst_w/2.0, 0], [dst_w/2.0, dst_h-1]])
    ...
    M = get_similarity_M(src_pts, dst_pts)
    ref_img_align = cv2.warpAffine(ref_img, M, dst_size, borderValue=(255, 255, 255), flags=cv2.INTER_LANCZOS4)
    pts = np.array(ref_image_pts[0]["body_points"])
    ...
    pose_video_pts_0 = np.array(pose_video_pts[0]["body_points"])
    ...
    M = get_similarity_M(src_pts, dst_pts)
    pose_imaegs_align = []
    for img_pil in pose_images:
        pose_img = img_pil.numpy() * 255
        pose_img_align = cv2.warpAffine(pose_img, M, dst_size, borderValue=(0, 0, 0))
        # pose_img_align = cv2.cvtColor(pose_img_align, cv2.COLOR_BGR2RGB)
        pose_img_align = np.array(pose_img_align, dtype=np.float32) / 255.0
        pose_imaegs_align.append(pose_img_align)
    
    # ref_img_align = cv2.cvtColor(ref_img_align, cv2.COLOR_BGR2RGB)
    ref_img_align = np.array(ref_img_align, dtype=np.float32) / 255.0

    return ref_img_align, np.array(pose_imaegs_align)
